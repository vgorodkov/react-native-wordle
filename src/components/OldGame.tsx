...
export interface UniqueLetters {
  correct: string[];
  inWord: string[];
  notInWord: string[];
}
...
const Game = () => {
  ...
  const isExistingWord = ALL_WORDS.includes(word);
          ...
          <WordRow
            ...
            key={
              currentRow.current === index
                ? activeCol === 0 || activeCol === 4
                  ? item.toString() + index
                  : index
                : index
            } //use this in oreder to trigger rerender when letter is deleted and activeCol is the same
